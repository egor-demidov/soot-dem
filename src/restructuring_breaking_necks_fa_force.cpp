...
using coating_model_t = filling_angle_coating_functor<Eigen::Vector3d, double>;
...
int main(int argc, const char ** argv) {
    ...
    print_header(parameter_store, "restructuring_breaking_necks_fa_force");

    if (parameter_store.simulation_type != "restructuring_breaking_necks_fa_force") {
        std::cerr << "Parameter file simulation type must be `restructuring_breaking_necks_fa_force`" << std::endl;
        exit(EXIT_FAILURE);
    }
