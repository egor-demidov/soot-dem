...
// File name: afm_breaking_necks.cpp
// File description: Driver program for AFM indentation simulations with dynamically breaking necks
...
int main(int argc, const char ** argv) {
    ...
    print_header(parameter_store, "afm_breaking_necks");

    if (parameter_store.simulation_type != "afm_breaking_necks") {
    ...
    // Neck strengths
    const double psi_n = get_real_parameter(parameter_store, "psi_n");
    const double psi_t = get_real_parameter(parameter_store, "psi_t");
    const double psi_r = get_real_parameter(parameter_store, "psi_r");
    const double psi_o = get_real_parameter(parameter_store, "psi_o");
...
//    const double afm_tip_t_max = /*2.0 * */t_tot * 0.018 /*2.1e-07*/;
    ...
    for (size_t n = 0; n < n_steps; n ++) {
        ...
        system.do_step(dt);
        break_strained_necks(aggregate_model, system.get_x(), k_n_bond, k_t_bond, k_r_bond, k_o_bond,
                             psi_n, psi_t, psi_r, psi_o, r_part);
